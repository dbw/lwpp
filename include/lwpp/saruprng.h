...
class Saru {
 public:
  ...
  /* inline */ Saru() : state(0x12345678), wstate(12345678) {};
  ...
  inline void setstate(unsigned int istate, unsigned int iwstate)
...
{
#if 0
  state  = 0x1fc4ce47*(seed^(seed>>13)); 
  wstate = (seed+0xcc00729f) ^ (state>>18); 
  state  = (state^wstate) + (wstate<<15); 
  wstate = (state+wstate) + (((signed int)state)>>27); 
  state  = state^(wstate<<3); 
  wstate = 0x856C4555+(wstate>>1); // 31 bits, within the offset Weyl range.
#endif
  //QualitySSARU_52_0.972_11_10_2_9_19_26_25_7_27_5_ade799b9_8c9db1b9_34ed42b5_6340fa2d_56dde7b5_7cd47a25_cfb60a4f_bfbc88b5_d4a7f783_4208a089.dat
  // Passes Crush with all 32 shifts, passes seedtest!
...
{
  state  = seed1+(seed2<<21);
  wstate = seed2+(((signed int)state)>>16);
  state  = state ^ (wstate>>10);
  state  = 0x1fc4ce47*(state^(state>>13)); 
  wstate = (wstate+0xcc00729f) ^ (state>>18); 
  state  = (state^wstate) + (wstate<<15); 
  wstate = (state+wstate) + (((signed int)state)>>27); 
  state  = state^(wstate<<3); 
  wstate = 0x856C4555+(wstate>>1);
}
...
inline Saru::Saru(unsigned int seed1, unsigned int seed2, unsigned int seed3) 
{
  ...
  state  = seed1+((~0xDEADBEAF)&seed3);
  wstate = seed2+(0xDEADBEAF&seed3);
  state  = 0x1fc4ce47*(state^(state>>13)); 
  wstate = (wstate+0xcc00729f) ^ (state>>18); 
  state  = (state^wstate) + (wstate<<15); 
  wstate = (state+wstate) + (((signed int)state)>>27); 
  state  = state^(wstate<<3); 
  wstate = 0x856C4555+(wstate>>1); // final is 63 bits of entropy
}
...
void inline Saru::advance(unsigned int steps)
{
...
template <unsigned int seed> inline 
